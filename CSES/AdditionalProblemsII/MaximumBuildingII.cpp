...
void solve(){
    ...
    for(int i = 1; i <= n; i++) {
        ...
        vector<pair<int, int>> unmark;

        for(int j = 1; j <= m; j++) {
            if(mark[nxt[j]][prev[j]]) continue;
            mark[nxt[j]][prev[j]] = 1; unmark.push_back({nxt[j], prev[j]});

            int h1 = max(1, max(v[nxt[j]], v[prev[j]]) + 1);
        ...
        for(auto [x, y] : unmark) mark[x][y] = 0;
    }
